...
class LRUCache:
    ...
    def get(self, key: int) -> int:
        print(f"Getting Cache value by Key: {key}")
        try:
            ...
            print(f"No value for Key: {key} was founded on Cache.")
            return -1

    def put(self, key: int, value: int) -> None:
        print(f"Putting object on Cache. Key: {key}, Value: {value}")
        try:
            key_value = self.lru_queue_cache.index(key)
            self.lru_queue_cache[key_value] = value
            print(f"Putting Updating Key Value: {key_value}")
            print(f"Putting Updated object in Cache: {self.lru_queue_cache[key_value]}")
        except ValueError:
            ...
            print(f"Putting object added to Cache: {self.lru_queue_cache}")
